...
@api_router.post("/audio_conversation", status_code=status.HTTP_201_CREATED)
async def process_conversation(
    request: ConversationRequest,
    db: AsyncSession = Depends(get_db)
):
    ...
    audio_bytes = audio_service.convert_to_audio(response)
    ...
    return {"ai_response": response, "state": state, "multiple_choices": multiple_choices, "audio_base64": audio_base64}
